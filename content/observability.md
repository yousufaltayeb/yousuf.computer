...
One thing the Logging Sucks article made me realize: you don't actually want to log 100% of everything. You **sample** — keep only a percentage.

A sensible default: 100% of errors, 100% of slow requests, 100% of anything from VIP customers, and 1–10% of the happy path. This is called **tail sampling** — you decide what to keep *after* the request finishes, so you never throw away the weird ones.
...
Logging and monitoring are how you *see* things going wrong. Alerting is how you get *told*, at 3am. You wire up a rule — "page me if error rate > 2% for 5 minutes" — in something like PagerDuty, Opsgenie, or Grafana Alerting. Alerting is its own skill (bad alerts → alert fatigue → ignored pages → outages nobody notices), but the basic loop is: metric crosses a threshold, alert fires, someone gets paged.

## Wrapping up

All these tools — Grafana, Sentry, OpenTelemetry, evlog, Datadog, and the rest — together they're called the observability stack. It's a whole world. I've barely scratched the surface.
...
- [Logging Sucks](https://loggingsucks.com/) — Boris Tane, the wide events / canonical log lines piece
